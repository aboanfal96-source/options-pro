...
async function fetchContractsFor(sym){
  ...
  try{
    ...
    const rsp=await fetch('/api/recommend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:sym,technicalScore:adj,direction,weights:AI.weights,expectedMove:sc.expectedMovePct,underlyingPrice:p,historicalVol:hv,fractal:fr})});
